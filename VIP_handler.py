...
for row in range(1, ws.max_row + 1):
    ...
    print(id_mapping)
    # CONFIGURE HERE
    checkstring = "00" + ws.cell(row, col_lastname).value[0] + ws.cell(row, col_firstname).value[0] \
                  + "000000"
    ...
    ws.cell(row, col_website).value = "https://www.csm-ev.com/members/vip/" + encoded.replace(".", "-")[
                                                                                   -75:].lower()
    ...
    if ws.cell(row, col_lastname).value + ws.cell(row, col_firstname).value not in name_list:
        ...
        ws_id.cell(temp_maxrow, col_birthday).value = "000000"
